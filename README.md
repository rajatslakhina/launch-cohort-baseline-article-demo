...
An illustrative 14-day migration window, read two ways — the blended line is a real weighted mix of the two cohorts beneath it, so it can never show a value outside what those cohorts actually did:

![Two-panel chart: left panel shows a blended launch-time p50 that looks like ordinary week-to-week drift; right panel shows the same 14 days segmented by OS cohort, where the iOS 26 line stays flat and the iOS 27 line climbs steadily the whole time](Demo/Screenshots/diagram.png)

Segmented by OS cohort, the picture changes completely. In the repo's own test fixture (not the illustration above, which is a smoothed 14-day version of the same idea) the blended fleet p50 moves a modest +5%. The iOS 27 cohort specifically — now the majority of the fleet — moved +23.5%. That's the masked regression `RegressionMasking.detect` exists to catch:
...
`Demo.xcodeproj` is a SwiftUI app that consumes the `LaunchCohort` package (as a local Swift package reference, same repo, so cloning this one repo and opening `Demo.xcodeproj` is the whole setup) and renders the exact same scenario `LaunchTelemetrySummaryTests.testDetectsMaskedRegressionAcrossAPlatformMigration` proves — same sample counts, same values, same +5%/+23.5% split — as a blended p50 card, a per-cohort trend list, and the flagged finding.
...
- **The live iOS Simulator run was skipped this run.** Before touching Xcode, this pipeline checks what's already on screen — and this time Xcode had a real, unrelated project open and actively debugging (a different app, mid-session, streaming live telemetry). Per this pipeline's own safety rule, it backed off immediately rather than risk disturbing that work, and is disclosing the skip here plainly instead of implying a Simulator screenshot exists. `Demo/Screenshots/` accordingly contains generated diagram and code-card images for the article, **not** a Simulator screenshot of the running app. `Demo.xcodeproj` and `LaunchCohortDemoView.swift` were **not** compiled this run — SwiftPM's `swift build`/`swift test` cover the library and tests only (`Demo/` isn't a SwiftPM target; Xcode is what builds it), so the SwiftUI view was reviewed by hand for correctness against the same `LaunchCohort` API the tests exercise, not verified by a real build. That distinction is intentional, not glossed over.
